...
| 蓝色 **Ad / 广告** | 这段内容是否包含直接销售、优惠或明确行动号召？ |
| 橙色 **Marketing / 营销** | 这段内容是否在为品牌、产品、服务或活动建立认知和需求？ |
| 粉色 **Soft Ad / 软广** | 这段内容是否把推广包装成个人体验、推荐或自然分享？ |
| 绿色 **Normal / 正常** | 前五项是否全部没有达到阈值？ |

前五项由 Jev 独立判断，可以同时出现；Normal / 正常由插件在前五项都未命中时得出。
...
页面右上角会显示检测数量，以及 AI、垃圾、广告、营销、软广和正常分别命中了多少条。这样，即使没有出现标签，也能区分“还在检测”和“这条没有达到标记阈值”。
...
核心判断交给 TypeSafe 的 Jev。每条正文在一次请求里回答五个独立问题，插件再根据返回的评分决定是否显示标签。
