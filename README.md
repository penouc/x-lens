...
**给 X 时间线加一层轻量提示。** 独立标记疑似 AI、slop、广告、营销和软广，并为全部未命中的内容显示正常，不替你屏蔽或隐藏内容。

![X Lens](docs/assets/blog-cover.png)

## 功能

- 紫色 **AI**、红色 **slop / 垃圾**、蓝色 **Ad / 广告**：独立判断，可同时出现。
- 检测当前可见推文和详情页文字评论，滚动时继续处理。
- 小标签放在 Grok 左侧；点击后在右上角查看本条评分。
- 默认阈值 65%，设置中可调。不是经过评测的准确率。
- 不访问作者主页、不创建后台标签页。自动屏蔽提示可配置，默认关闭，最终屏蔽仍需你确认。
...
设置中勾选“自动弹出屏蔽提示”并保存。只有 slop 触发，AI 或广告单独命中不会触发。同一页面每个账号只提示一次，提示至少间隔 30 秒；刷新页面重新计数。点击屏蔽后还需在 X 原生窗口确认。取消勾选并保存会关闭当前提示。
