...
const autoBlockPrompt = document.querySelector('#autoBlockPrompt');
const value = document.querySelector('#value'), status = document.querySelector('#status');
chrome.storage.local.get(['apiKey','enabled','labelThreshold','lastError','autoBlockPrompt']).then(data => { autoBlockPrompt.checked = data.autoBlockPrompt === true; key.value = data.apiKey || ''; enabled.checked = data.enabled || false; threshold.value = Math.round((data.labelThreshold || 0.65) * 100); value.textContent = threshold.value + '%'; document.querySelector('#error').textContent = data.lastError || ''; });
...
document.querySelector('form').addEventListener('submit', async event => {
  ...
  await chrome.storage.local.set({apiKey: key.value.trim(), enabled: enabled.checked, labelThreshold: Number(threshold.value)/100, autoBlockPrompt: autoBlockPrompt.checked});
